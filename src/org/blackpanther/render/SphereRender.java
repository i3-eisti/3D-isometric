...
public class SphereRender extends AbstractRenderer<Sphere> {
    ...
    protected final void modelRender(final BufferedImage _buffer) {
        ...
        for (int abscissa = getBufferWidth(); abscissa-- > 0; ) {
            for (int ordinate = getBufferHeight(); ordinate-- > 0; ) {
                ...
                if (withinSphere(sphereReferential, radius)) {
                    ...
                    if (lessThanPIByTwo || lessThanMinusPIByTwo) {
                        ...
                        incidence = normal.angle(lightVector);
                        rawColorStrength = (float) cos(incidence);
                    } else {
...
//                    painter.setColor(renderedColor);
//                    painter.drawLine(abscissa, ordinate, abscissa + 1, ordinate + 1);
                    _buffer.setRGB(abscissa, ordinate, renderedColor.getRGB());
                }
            }
        }
    ...
    class IndividualLightRender extends RGBImageFilter {

        public IndividualLightRender() {
        ...
        public int filterRGB(int x, int y, int rgb) {
            ...
            if (withinSphere(sphereReferential, radius)) {
                ...
                alpha = (rgb >> 24) & 0xff;
                red = ((int) (sphereColor.getRed() * colorStrength) >> 16) & 0xff;
                green = ((int) (sphereColor.getGreen() * colorStrength) >> 8) & 0xff;
                blue = ((int) (sphereColor.getBlue() * colorStrength)) & 0xff;
